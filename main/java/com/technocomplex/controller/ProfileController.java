...
public class ProfileController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
		    ...
		    //rest
			System.out.println("Profile VAlidtion "+Arrays.toString(validationMessage));
            //TODO:: change the length to count for easer
			String[] filteredArray = Arrays.stream(validationMessage)
			      .filter(x -> x != null)
			      .toArray(String[]::new); // return string array  without null
			System.out.println("Filter"+Arrays.toString(filteredArray));
			System.out.println("Filter size"+filteredArray.length);
			if(filteredArray.length != 0) { // validation message is present
			...
			Boolean isAdded = profileService.updateUser(userModel); //TODO: change the student name to user
	...
	private boolean uploadImage(HttpServletRequest req) throws IOException, ServletException {
		Part image = req.getPart("profile");
		return imageUtil.uploadImage(image, req.getServletContext().getRealPath("/"), "customer"); //TODO: change to user
	}
